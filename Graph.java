...
public class CS400Graph<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        //check if frontier is null/doesn't contain vertices
        if(start == null || end == null || !containsVertex(start) || !containsVertex(end)) {
            throw new NoSuchElementException();
        }
        //priority queue acts as frontier
        PriorityQueue<Path> frontier = new PriorityQueue<>();
        //make vertices out of starting node and ending node
        Vertex last = vertices.get(end);
        //add start path to frontier
        frontier.add(new Path(vertices.get(start)));
        List<Vertex> visited = new LinkedList<>();

        while(!frontier.isEmpty()) {
            Path lowestPath = frontier.peek();
            Vertex current = lowestPath.end;
            visited.add(current);

            //if the current node is the last node, the shortest path has been found
            if(current == last) {
                return lowestPath;
            }

            //current vertex is visited
            frontier.remove(lowestPath);

            //iterate through neighbors
            for (Edge edge : current.edgesLeaving) {
                //make new paths
                Path extended = new Path(lowestPath, edge);
                if (!visited.contains(extended.end)) {
                    frontier.add(extended);
                }
            }
        }
        //if last hasn't been found
        throw new NoSuchElementException();
    }
